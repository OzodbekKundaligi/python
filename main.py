...
# Environment variables o'qiymiz
load_dotenv()
...
def init_db():
    ...
    try:
        ...
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_startups_owner_id ON startups(owner_id)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_startups_status ON startups(status)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_startup_members_startup_id ON startup_members(startup_id)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_startup_members_user_id ON startup_members(user_id)')
        
        conn.commit()
        logging.info("✅ PostgreSQL database muvaffaqiyatli ishga tushirildi")
...
def save_user(user_id: int, username: str, first_name: str):
    ...
    try:
        ...
        logging.error(f"save_user xatosi: {e}")
        conn.rollback()
    finally:
        cursor.close()
        conn.close()
...
def create_startup(name: str, description: str, logo: str, group_link: str, owner_id: int) -> int:
    ...
    try:
        ...
        startup_id = cursor.fetchone()[0]
        conn.commit()
        return startup_id
    except Exception as e:
        logging.error(f"create_startup xatosi: {e}")
        conn.rollback()
        return -1
    finally:
        cursor.close()
...
def update_startup_status(startup_id: int, status: str):
    ...
    try:
        if status == 'active':
            cursor.execute('''
                UPDATE startups SET status = %s, started_at = CURRENT_TIMESTAMP 
                WHERE startup_id = %s
            ''', (status, startup_id))
        elif status == 'completed':
            cursor.execute('''
                UPDATE startups SET status = %s, ended_at = CURRENT_TIMESTAMP 
                WHERE startup_id = %s
            ''', (status, startup_id))
        else:
            cursor.execute('UPDATE startups SET status = %s WHERE startup_id = %s', (status, startup_id))
        
        conn.commit()
    except Exception as e:
        logging.error(f"update_startup_status xatosi: {e}")
        conn.rollback()
    finally:
        cursor.close()
        conn.close()
...
def add_startup_member(startup_id: int, user_id: int):
    ...
    try:
        ...
        logging.error(f"add_startup_member xatosi: {e}")
        conn.rollback()
    finally:
        cursor.close()
        conn.close()
...
def get_join_request_id(startup_id: int, user_id: int):
    ...
    try:
        ...
        logging.error(f"get_join_request_id xatosi: {e}")
        return None
    finally:
        cursor.close()
        conn.close()
...
def update_join_request(request_id: int, status: str):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute('UPDATE startup_members SET status = %s WHERE id = %s', (status, request_id))
        conn.commit()
    except Exception as e:
        logging.error(f"update_join_request xatosi: {e}")
        conn.rollback()
    finally:
        cursor.close()
        conn.close()
...
def get_statistics() -> Dict:
    ...
    try:
        ...
        cursor.execute('SELECT COUNT(*) FROM startups WHERE status = %s', ('active',))
        stats['active_startups'] = cursor.fetchone()[0]
        
        cursor.execute('SELECT COUNT(*) FROM startups WHERE status = %s', ('pending',))
        stats['pending_startups'] = cursor.fetchone()[0]
        
        cursor.execute('SELECT COUNT(*) FROM startups WHERE status = %s', ('completed',))
        stats['completed_startups'] = cursor.fetchone()[0]
        
        cursor.execute('SELECT COUNT(*) FROM startups WHERE status = %s', ('rejected',))
        stats['rejected_startups'] = cursor.fetchone()[0]
        
        return stats
    except Exception as e:
        logging.error(f"get_statistics xatosi: {e}")
        return {'total_users': 0, 'total_startups': 0, 'active_startups': 0, 
                'pending_startups': 0, 'completed_startups': 0, 'rejected_startups': 0}
    finally:
        cursor.close()
        conn.close()
...
def approve_join_request(call):
    try:
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute('SELECT startup_id, user_id FROM startup_members WHERE id = %s', (request_id,))
        result = cursor.fetchone()
...
def reject_join_request(call):
    try:
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute('SELECT user_id FROM startup_members WHERE id = %s', (request_id,))
        result = cursor.fetchone()
...
def process_startup_photo(message, startup_id, results_text):
    ...
    if message.photo:
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute('UPDATE startups SET status = %s, results = %s, ended_at = CURRENT_TIMESTAMP WHERE startup_id = %s', 
                      ('completed', results_text, startup_id))
        conn.commit()
